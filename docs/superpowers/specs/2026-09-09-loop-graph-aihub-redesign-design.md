...
## 7A. 产品要求（用户补充，2026-09-09，贯穿各期）

以下五条为正式需求，优先级与分期标注在每条末尾。

### R1. 聊天每日自动 Brief（P2）

每个活跃 Matrix 房间/频道每天自动生成一份简报：昨日 run 进展（完成/失败/卡点）、待办审批、今日计划重入的 loop。实现为**系统内置的每日定时 GraphSpec 模板**（cron 重入 + summarize 节点 + persistence 节点投递到 Matrix 房间），不是独立子系统——这同时是"调度器 + LLM 节点 + Matrix 投影"的端到端样板。简报样式克制成三段式（进展 / 等你决策 / 今日计划），不刷屏。

### R2. Workspace 上下文注入（P1）

节点进入 workspace（agent 节点拉起 Claude Code / Codex 终端或派发 subagent）时，必须把**目标、当前进度、图上下文**注入工作区，Agent 落地不能是空白目录。注入内容物化为 workspace 根的 `GRAPH-CONTEXT.md`：

```markdown
# 任务上下文（自动生成，勿手改）
- 目标（goal）：<图目标>
- 当前节点：<nodeId/label>，第 <iteration> 轮迭代
- 上游产出：<前驱节点 update 的摘要>
- 完成判定：<stopCondition / 本节点 expectedOutputs>
- 剩余预算：<__remainingSteps / maxCost 余量>
- 图回放：<run 详情页深链>
```

由 agent/loop-phase 节点在执行前自动写入 workspace；Claude Code 经 `CLAUDE.md` 引用该文件（追加一行 `@GRAPH-CONTEXT.md`），Codex 经 AGENTS.md 同理。上游摘要由 runtime 从事件日志汇聚，不需要节点作者手工拼装。

### R3. 质量门禁内建为图节点（P1 起，P4 模板化）

质量门禁不是外部 CI 的概念移植，而是**图结构的一部分**：编译器对五阶段模板默认在 `persistence → scheduling` 之间插入 `gate` 节点（test/lint/build/契约校验，命令可配置）；`gate` 失败 → 守卫回边回 `handoff`（repair 语义不变）；P4 编排器把"质量门禁"做成可拖入的一等节点模板（内置命令白名单 + 超时 + 失败路由配置）。这样"质量门禁做到 workflow 的要求里"是可编排、可观测、可回放的。

### R4. 人的使用习惯与感受（P3 验收标准）

- **首屏不空**：登录首屏（总览）默认有内容——今日 brief、活跃 run、待办审批；零数据时引导创建第一个图（从模板，三步内跑起来）。
- **通知克制**：默认只推"等你决策"（interrupt/审批/失败熔断）；节点成功不推送；每日 brief 一次；同 run 的失败通知 5 分钟内合并。
- **倒序时效**：运行列表按"最后活动"排序；正在进行中的 run 有活动心跳（最后事件距今）。
- **键盘可达**：运行中心支持 j/k 移动、Enter 打开、r 回放、a 审批。
- **深色/浅色跟随系统**；新 IA 沿用 Pure Ink 主题，不为图编辑器引入新色板（节点状态只用 error/warning/success 三色 + 灰阶）。
- **中文优先**的默认文案；所有新 UI 文案走 i18n（现有 patch 体系）。

### R5. 工程化门槛（贯穿，作为完成定义）

- **可观测**：每 run 有完整事件流 + 结构化日志 + 成本/时长指标；`npm test` 全绿是每期收口门禁。
- **错误可懂**：节点失败显示人类可读原因 + 建议动作（重试/跳过/编辑图），不显示裸堆栈。
- **性能预算**：运行列表 1000 run 内滚动不卡（虚拟滚动）；执行图 500 节点内流畅（Vue Flow 节点懒渲染）。
- **数据不丢**：所有用户可见状态来自事件日志回放或 checkpoint，内存态可随时重建。
- **无僵尸**：每期收口清理当期产生的孤儿组件/死代码（延续 batch 纪律）。
- **文档同步**：README 功能特色章节随 P3 落地更新为新 IA；release notes 每期产出。
...
| **P1 切换** | 编译器 + 生产装配切换（patch 134 重接）+ 四大断链修复 + 适配器降级投影 + 旧数据迁移 + 调度器收敛 + **R2 workspace 上下文注入** + **R3 质量门禁 gate 节点内建** | 既有 loop 集成测试在编译产物上通过；双跑对比（新旧引擎同输入同事件序列）；`npm run build:full` 通过 |
| **P2 运行中心** | 运行列表/详情、执行图画布、时间轴回放、节点检查器、`/graph` Socket 流 + **R1 每日聊天 Brief（系统内置 cron 图模板）** | 端到端：创建 loop → 观察执行 → 介入审批 → 回放导出；brief 模板每日投递到 Matrix |
| **P3 信息架构** | 新导航六区域、总览、介入收件箱、工作项关联、Matrix 升一级、观察者聚合、store 拆分、旧 cockpit 退役 + **R4 体验验收（首屏不空/通知克制/键盘可达/主题/i18n）** | 契约测试扩展 + 全路由回归；旧 cockpit 路由下线；R4 逐条走查 |
| **P4 编排器** | 画布编辑、节点面板、loop 容器、编辑守卫、模板库（含质量门禁节点模板 R3）、导入导出 | 从空白画布拖出”循环内含审批”的图并成功运行 |
