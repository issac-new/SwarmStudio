...
- **下一实施批（按优先级）**：~~P0 十项~~ → **P0 已落 3/10**（recap 域 v1+来源标头防冒充=patch 410；成本估算价目表 dsh 三原则=pricing.ts+runtime/pricing/）→ 余 P0 七项（squad leader 协议/列级编排/任务证据累积//review/任务结果卡+changed-files）→ P1 二十项 → 旧账随迁批（📦9 项）→ 核验批。
