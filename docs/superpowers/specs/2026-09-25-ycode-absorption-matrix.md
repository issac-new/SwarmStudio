...
4. 待排期 8 项建议落点：#1→IdeRoundsPane；#3→IdeMcpPane 技能 tab；#5→loop run-line 接线；#6→contextBreakdown 六源网格；~~#7→IdeGoalBudgetFloat 三预算字段~~（✅goal-budget.ts）；~~#6→contextBreakdown 六源网格~~（✅context-six-source.ts）；~~#1→IdeRoundsPane~~（✅rounds-table.ts）；~~#8~~（✅usage-ledger.ts）；~~#5→loop run-line 接线~~（✅workflow-run-line.ts：状态机/节点进度/产物）；#9→ChatInput @ 域合并（衔接派单半边）；#22→导入器换源（codex/kimi）。**待排期 3 项余**（#3 技能入口/#9/#22）。
