...
- **下一实施批（按优先级）**：~~P0 十项~~ → **P0 已落 4/10**（recap 域 v1+来源标头=410；价目表 dsh 三原则=pricing.ts；squad leader 协调协议=squad-protocol.ts 四件语义）→ 余 P0 六项（列级编排/任务证据累积//review/任务结果卡+changed-files）→ P1 二十项 → 旧账随迁批（📦9 项）→ 核验批。
