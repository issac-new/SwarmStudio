...
- **下一实施批（按优先级）**：~~P0 十项~~ → **P0 已落 8/10**（recap 域 v1+来源标头=410；价目表=pricing.ts；squad leader 协议=squad-protocol.ts；任务证据累积=patch 413；任务结果卡=result-card.ts；/review 评审域=patch 414；列级 automation 编排=patch 415 配置+匹配器）→ **P0 十项全部落地**（最后一项=列编排执行半环 dispatchColumnTransition：匹配→派单，事件自动触发挂 loop 域列后续扩面）→ P1 二十项 → 旧账随迁批（📦9 项）→ 核验批。
