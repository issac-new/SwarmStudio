...
# 导演脚本只扮演"人类打字"（含审批回复）；所有 agent 动作为真实 gateway + LLM 回合。
#
# 等待策略（09-17 实测修订）：agent 会先发进度消息、或不按指令放完成标志，
# 因此等待一律轮询"地面真值"（central 的分支/文件/tag），消息流只用于留证与
# 代人类回复 !approve。START_STEP 支持断点续跑：
...
if step_reached rfd; then
  if [[ -z "$(sget rfd_marker)" ]]; then
    M1=$(mx_send "$ALICE" "$ROOM_ID" "【需求 RFD-001】stringops v0.1
...
- 你的工作区（本机检出）: ${WS_BOB}（remote central 已指向共享裸仓）
...
- 测试命令（必须跑通）: cd ${WS_BOB} && ${PY} -m pytest -q
...
- 注意：不要把 __pycache__/*.pyc 提交进仓库；结论行以 IMPL-DONE-T2 或 IMPL-FAIL-T2 开头。不许谎报。"
  wait_truth "central 有 feat/slugify 与 feat/truncate" 1800 truth_impl \
    || fail "实现阶段超时：分支未就绪（房间消息与 gateway 日志为准）"
...
- 全绿: git push central main，结论行以 VERIFY-PASS 开头并附用例数；任何失败: 结论行以 VERIFY-FAIL 开头，不要谎报。"
  ...
  # 导演地面真值复跑：在 alice 工作区独立跑 pytest
  (
    cd "$WS_ALICE"
    git fetch -q central
...
note "场景完成 ✅  运行 bash sim-evidence.sh 取证"
