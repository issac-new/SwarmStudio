...
│   ├── hermes-studio/        #   SwarmStudio 桌面应用主体（v0.6.23）
...
| 上游项目 | GitHub 仓库 | 用途 |
|---------|-----------|------|
| **hermes-studio** | https://github.com/EKKOLearnAI/hermes-studio | SwarmStudio 桌面应用主体（Vue 前端 + Koa 后端 + Electron 壳），本 overlay 的注入目标（v0.6.23） |
...
SwarmStudio 桌面端当前版本 **0.6.23**，构建产物分 **macOS** 与 **Windows** 两个版本。
...
#       ├── SwarmStudio-0.6.23-arm64.dmg
#       └── SwarmStudio-0.6.23-arm64.zip

# Windows 版（x64 exe + zip + msi）
npm run build:dmg:win
# 产物：upstream/hermes-studio/packages/desktop/release/
#       ├── SwarmStudio-0.6.23-x64.exe
#       ├── SwarmStudio-0.6.23-x64.zip
#       └── __msi-x64/
```
...
- **43** 个单测文件
- 上游基础：hermes-studio v0.6.23 / hermes-agent / element-web v1.12.22
