...
## 版本
SwarmStudio **2.33**（基于 hermes-studio v0.7.24 + hermes-agent v0.21.4 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.4）
...
> **2.33 — hermes-studio v0.7.24 升级轮（2026-09-22 深夜）** — main 演进 351f4b5 → feat/upstream-0.7.24 合入（916f30f）。**上游**：hermes-studio **v0.7.23 → v0.7.24**（31 commits / 202 文件 +8743/−940：iOS Live Activities 推送编排 + 通知内容预览 + App 侧会话共享（sessionShare 授权面）+ codex 模型设置按 profile 过滤 + coding agent 压缩超限恢复 + npm 发版 tarball 本地化 + Studio 仓库引用迁 ekko-studio）；element-web **v1.12.28 → v1.12.29**；hermes-agent **v0.21.4 不动**（远端最新 tag v2026.9.21 即当前基线），desktop 捆绑 runtime pin 0.21.4 不变。**patch regen 4**：042（v0.7.24 纯净 JSON 基座字段级品牌重建——版本行随上游 0.7.24，desktop 产物名随之带 0.7.24）；097（download.ts+files.ts——上游 sessionShare 授权分支与我方 root 沙箱分支合并为 sessionShare→isAbsolute→root 沙箱→profile 相对四路分支）；123（任务 workspace 白名单叠加 sessionShare 分支：越界放行语义与分享授权共存）；329（品牌扫描 × 上游 ekko-studio 仓库迁移——GitHub 仓库路径取上游、资产名品牌取我方）。**新增 367**：AuthenticatedUser.matrix_user_id 放宽为 string|null——上游 #3144 起新通知族（live-activity/run-push/catchup）把 UserRecord（015 可空列）直传 canReceiveAppEvent（AuthenticatedUser），016 的 ?:string 结构不兼容，消费面核查全 null 容忍后放宽。**门禁**：/tmp 纯净树全系列重放 hs 侧 302/302（agent 6 patch 路由树外）+ 终态与逐 patch 提交树 diff 一致；inject 308/308（含并行会话 365/366 合入）；server tsc 0 错；overlay vitest **230 文件 2226/2226**；i18n-coverage 18/18；build:full EXIT=0。**产物（用户指令不上传 GitHub/ModelScope）**：arm64 dmg 399,590,713B sha256 `44a03464c2259789…`（无签名 adhoc，TeamIdentifier=not set）+ win x64 zip（同轮构建，指纹见 release 注记）。本机 /Applications 已换装。
