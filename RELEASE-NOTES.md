...
## 版本
SwarmStudio **2.33**（基于 hermes-studio v0.7.24 + hermes-agent v0.21.4 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3=上游已发布最新，见 2.33 详段 pin 修正说明）
SwarmStudio **2.32**（基于 hermes-studio v0.7.23 + hermes-agent v0.21.4 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.4）
SwarmStudio **2.31**（基于 hermes-studio v0.7.23 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.30**（基于 hermes-studio v0.7.23 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.29**（基于 hermes-studio v0.7.23 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.28**（基于 hermes-studio v0.7.22 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.27**（基于 hermes-studio v0.7.22 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.26**（基于 hermes-studio v0.7.22 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.25**（基于 hermes-studio v0.7.22 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.24**（基于 hermes-studio v0.7.22 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.3）
SwarmStudio **2.23**（基于 hermes-studio v0.7.21 + hermes-agent v0.21.3 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.0）
SwarmStudio **2.22**（基于 hermes-studio v0.7.21 + hermes-agent v0.21.2 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.0）

> **2.33 — hermes-studio v0.7.24 升级轮（2026-09-22 深夜）** — main 演进 351f4b5 → feat/upstream-0.7.24 合入（916f30f）。**上游**：hermes-studio **v0.7.23 → v0.7.24**（31 commits / 202 文件 +8743/−940：iOS Live Activities 推送编排 + 通知内容预览 + App 侧会话共享（sessionShare 授权面）+ codex 模型设置按 profile 过滤 + coding agent 压缩超限恢复 + npm 发版 tarball 本地化 + Studio 仓库引用迁 ekko-studio）；element-web **v1.12.28 → v1.12.29**；hermes-agent **v0.21.4 不动**（远端最新 tag v2026.9.21 即当前基线）。**desktop 捆绑 runtime pin 修正为 0.21.3（hermes-0.21.3-runtime，上游已发布最新 runtime 资产；v2026.9.14/345cd2b0）**——2.32 轮曾把 pin 指到 hermes-0.21.4-runtime，但上游从未发布该 runtime 资产（runtime 缺失场景首启下载必 404），本轮实测发现后回退；agent 源码跟踪维持 0.21.4，runtime 待上游发版后随轮升。**patch regen 4**：042（v0.7.24 纯净 JSON 基座字段级品牌重建——版本行随上游 0.7.24，desktop 产物名随之带 0.7.24）；097（download.ts+files.ts——上游 sessionShare 授权分支与我方 root 沙箱分支合并为 sessionShare→isAbsolute→root 沙箱→profile 相对四路分支）；123（任务 workspace 白名单叠加 sessionShare 分支：越界放行语义与分享授权共存）；329（品牌扫描 × 上游 ekko-studio 仓库迁移——GitHub 仓库路径取上游、资产名品牌取我方）。**新增 367**：AuthenticatedUser.matrix_user_id 放宽为 string|null——上游 #3144 起新通知族（live-activity/run-push/catchup）把 UserRecord（015 可空列）直传 canReceiveAppEvent（AuthenticatedUser），016 的 ?:string 结构不兼容，消费面核查全 null 容忍后放宽。**门禁**：/tmp 纯净树全系列重放 hs 侧 302/302（agent 6 patch 路由树外）+ 终态与逐 patch 提交树 diff 一致；inject 308/308（含并行会话 365/366 合入）；server tsc 0 错；overlay vitest **230 文件 2226/2226**；i18n-coverage 18/18；build:full EXIT=0。**产物（用户指令不上传 GitHub/ModelScope）**：arm64 dmg 399,634,592B sha256 `5d0c9a22083e0a05…`（无签名 adhoc，TeamIdentifier=not set；pin 0.21.3 修正版）+ win x64 zip 430,707,809B sha256 `204becccd21fefcf…`（unzip -t 零错；nsis -86 在案惯例非交付物）。本机 /Applications 已换装（三验：CFBundle 0.7.24 + 存活 + :8748 livez ok + active-version desktopAppVersion=0.7.24/hermesRuntimeVersion=0.21.3 对齐）。

> **2.32 — hermes-agent v0.21.4 升级轮 + R6 尾巴缺陷清偿（2026-09-22）** — 上游面：hermes-studio **v0.7.23** 保持 GitHub Latest（v1.0.x tags 为 Android/HStudio 其他产品线，按 gh release Latest 标记实核不受影响）；element-web **v1.12.28** 保持；hermes-agent **v0.21.3 → v0.21.4**（tag v2026.9.21，5173 commits / 5170 文件上游增量——2.31 时预告的"前移 4947 commits 未发布线"现已发布）。①**agent patch 面**：6 patch 中 2 regen——118（上游 create_profile 的 `_bootstrap_profile_dir` 新增第三参 `sync_imports`，我方 run-trace seed 块随迁保留）+ 178（`_TASK_DICT_FIELDS` 上游新增 `max_runtime_seconds` 与我方 `reasoning_effort` 相邻共存）；117/179/250/273 干净套用零 regen；117 语义实核（workspace_kind/project link 逻辑上游零改动）。Phase 0 纪律：纯净 v2026.9.21 树顺序套用 6/6 干净 + 渐进树三方合并 + 双树 diff 为空。②**main 预存缺陷清偿 3 处（本轮全新注入抓获）**——**patch 357 重复 hunk**（R6 会话在无 001 的错误基线上产出两态 diff，携带 users matrix 列 + UNIQUE 迁移守卫两个 001 已有 hunk，main 自 R6 合入起 clean+inject 链路即断，重造为单 hunk 租约两列）；**mention-bus.ts 相对深度错**（`../loop` 应为 `../../loop`，vitest 只直测 engine 层从未走到 controller，仅 server tsc 可抓获）；**squad-leader.ts `@/custom` 别名断裂**（server tsc 符号链接视角不可达 custom/client，AgentRosterRow 本地声明 + 指针注释）。③**inject.mjs**：脏检查豁免 `?? .loop/` 运行态目录（graph-events.db 由运行中 server 持有句柄，app 运行期间每次 inject 必撞）。④**patch 207**：runtime pin 三元组升 0.21.4 / v2026.9.21 / dd2c35f4。⑤上游依赖面：uvicorn [standard] 拆分（httptools/watchfiles 转核心、uvloop 变 optional extra），对 venv 无破坏性。**门禁终态**：inject **300/300**；server tsc **0 错**；overlay vitest **218 文件 2159 过/0 fail**；上游 i18n-coverage **18/18**；agent pytest hermes_cli+gateway 焦点组 **3024 过/19 skip/1 败**（1 败=test_dashboard_param_clamps，clamps 家族工作树环境互作在案定性：隔离重跑过 + 纯净 v0.21.4 重放树 9/9 过双对照，2.24 轮同族）；build:full **EXIT=0**（openapi→vite→tsc→server 全链）。**产物（按用户指令不上传 GitHub/ModelScope，仅本地构建物）**：mac arm64 dmg **399,602,916B** sha256 `6e136fd8549e47d7…`（09-22 13:32 构建，同名覆写 2.31 的 a699e472；TeamIdentifier=not set 无签名；CFBundleShortVersionString 0.7.23；**build/runtime-release.json 实证 pin 三元组 tag=hermes-0.21.4-runtime / ref=v2026.9.21 / commit=dd2c35f4**）+ win x64 zip **430,651,637B** sha256 `9157de163eefb000…`（13:34 构建；unzip -t 零错；`.vite` 零条目 = 281 生效）。旧 2.31 的 mac zip/nsis 残留未清理（非交付物）。

> **2.31 — 矩阵协作 M 轮 + 驾驶舱 v12.2→v12.7 + kanban 状态机诚实化（2026-09-21）** — 三上游零升级（hermes-studio **v0.7.23** / hermes-agent **v0.21.3** / element-web **v1.12.28** 全 GitHub Latest 实核；上游 main 已前移但无新稳定 tag——hs 前移 22 commits 至 iOS Live Activities #3111、agent 前移 4947 commits，均未发布线，稳定版口径不动）。本轮内容 = main 自 2.30 说明（f425eb4）后 61 提交合流。①**矩阵协作 M-A..M-G 七轮大主线（matrix-teams 协议 v2）**——架构 spec v1→v1.2 三裁决代拟（A 案例房事件流为准 / B 一机一集群+能力标签 / C @user-agent 单账号；会签/甘特/催办入 v1）+ 注册通道 runbook 实测入库；协议 v2 落码（projectId/R 门/signoffs/agent.profile）+ 既有 31 例升 v2 断言（135/135 绿）；M-B agent-router 决策表 + maxParallel 排队重试 + 任务树拆分；M-C 评审中心（R/G 门评审卡片三态 + 会签 signoffs 逐人渲染 + 管理台评审区）；M-D 消息任务联动（消息转任务/看板态投影/卡片操作写路径/甘特投影+面板，15 例绿）；M-E IDE 接线（任务上下文条：标题/状态/文档指针/聊天锚点/ACP 绑定）；M-F 测试统计催办（缺陷自动建 parentId 挂原任务 + 四维统计投影）；M-G sim-fullchain 真实 Synapse 全链协议验证（case/stage/G 门/R 门会议流）。②**C1/C2 收口**——浅色色块根治 + 全局顶区常驻双视图（v12.1 壳层重排）+ review 态 CLI 桥（322/323：complete-allow-review / reopen-review-bridge）+ 右栏动作按任务所在板定位（按钮失效根治）+ 态势条五项跳转 + review 出口双桥 + 319 review 分支 await/return 语义修正。③**IDE 工作台重构**——IdeChatPane 浮窗化（IdeFloatPanel 拖拽壳）+ 链路维度退役 + IdeTopBar 删除 + files 页签 + 守门测试（vi.hoisted 数据工厂坑修复）+ i18n patch 327。④**驾驶舱 v12.2→v12.7 六轮迭代**——v12.2 态势条内联面板 + 视图切换器入页头（326）；v12.3 五轮（通知模态与最小化 dock 链路退役/注意力条管理入口/态势条迁页头收口/工作台三栏栏控折叠/在线态势段两级级联/左栏聚类任务簇双击动线/三聊天合一（320 键块归位）/会话工作台面板四块/i18n 328 + ia2 覆盖率守门）；v12.4 七项裁定（全局品牌 Ekko Studio→SwarmStudio **patch 329**/语言/单按钮/等我对齐/栏控迁三栏）；v12.5/v12.6 十项裁定（332/333：聚合看板修全 0=rawTasks 数据源+板多选排除集/任务下拉跳转 ide?task/在线三级级联 profile 连接键/栏控叠放/IDE 维度条+侧栏 footer 退役）；v12.6① 纠正（0dd67d2：关闭钮落打开的看板页 TasksView ia-board-close→ia2.collab；注意力条误加回滚）；v12.7 顶栏对齐根治 + 任务按钮全状态分类汇总（169fd5b）。⑤**kanban 状态机诚实化（patch 334/335）**——DELETE 走 archive+--rm 两步硬删（CLI 无 rm 动词此前必 500）+ bulk/patch 链补 scheduled/todo/triage 状态迁移（不可达组合抛清晰错误）+ moveTask 回检 bulk results 失败抛错不再假成功；修复在案缺口：PATCH status:'todo' 静默无操作 + review 任务 DELETE 500。⑥**构建与 patch 卫生**——版本号随 2.31 补齐（2.30/2.31 两轮欠账，e8fcb8a）/328 头部重产（diff -u 仅两行头错位修复，edf14f4）/kanban 自定义组件缺键补全（324/325 zh/en 各 +151 键 + 零缺键守门）。**门禁终态**：overlay vitest **192 文件 1914 过/0 fail**；上游 i18n-coverage **18/18**；overlay build:full EXIT=0（server tsc 0 错）；desktop build:main 0 错。**产物（本轮按用户指令不上传 GitHub/ModelScope，仅本地构建物）**：mac arm64 dmg **399,570,323B** sha256 `a699e472bb0cbfa0…`（12:31 全新构建，≠2.30 的 ce35f320；无签名 adhoc；CFBundleShortVersionString 0.7.23；webui/dist 345 js chunk、WorkbenchView-Be5lwmgN 在产、v12 内容锚 ia2.nav.collab/boardClose/task-card 分类态实测命中；`.vite` 零条目 = 281 生效）+ win x64 zip **430,617,768B** sha256 `1c1b8b273aabea9c…`（12:33 全新构建，≠2.30 的 da62c780；unzip -t 零错；nsis 步 `spawn -86` bad CPU type 失败——历史在案惯例纯 zip 规避，非交付物）。
> **2.30 — v12 统一视图智能驾驶舱轮（2026-09-19）** — 三上游零升级（hermes-studio **v0.7.23** / hermes-agent **v0.21.3**；desktop 捆绑 runtime 0.21.3 不动）。本轮内容 = main 自 2.29（cd75e3a）后 45 提交合流。①**v12 统一视图（主体，13 提交）**——设计稿 v12（会话×循环×任务×参与者四对象模型）整稿实施：六场景导航退役，/app 收敛双视图（沟通协作工作台 + IDE 工作台直链）；沟通协作三栏 250/自适应/240 铁律——左栏工作流导航（matrix 房间∪hermes 会话∪循环统一列表：过滤 chips/任务号搜索/阶段进度分段条/未读与 📋 挂接徽章）、中栏对象工作区（会话→对话画布[MatrixRoomCanvas=房间头+消息面板+右信息面板，无房间列表]+链路条[对象→挂接任务→等你门]+参与方条[👤∪🤖∪值守]；循环→运行画布[实时=阶段流①-⑤+最新 run 图+LIVE 徽章；历史=j/k 键步进回放+事件编年 verbose+exportRun 导出归档]）、右栏任务与决策恒驻（等我三源聚合[review 任务验收/打回·awaiting-input 运行确认·fleet 审批确认]与壳层注意力条同源+挂接任务改派[看板抽屉复用]/⌨/去处理+任务动态流）；壳层态势条（等我⚠最久/任务/会话/循环阻塞/在线明细+⚙管理）；⚙管理台全局覆盖层五区（任务过滤表[详情/⌨/历史行上操作]、会话管理[30 天无活动归档候选标记]、员工编制负载、智能体 fleet∪agentTeam 档案、团队与通道卡[platforms store 共享探测 retain/release 引用计数]）+详情面板（实时卡+↗运行·历史/⌨ IDE 任务空间）；IDE 工作空间维度条（任务/项目/会话/链路四维度+⇄沟通协作+/ide?task= 深链）；路由迁移表 12 条路径重定向（/app/ops→/app/runs、/app/tasks→/app/board、/app/comms→/app 等；含参目标函数式重定向——vue-router 字符串重定向不透传参数）；退役名 ia2.overview/ops/tasks/comms 守门零残留，ia2.collabSession/commsRoom 参数名原样（深链免改）；**新增 patch 316/317**（v12 全量文案 ~120 键×双语成对，ia2.nav.collab 值改「沟通协作」；裸 @ 经 i18n-compile 门禁抓获转义 {'@'}）。②**IDE 批（并行会话合流）**——布局 v2/v3（中栏删除+终端进右辅助面板+色系回归+侧栏修复×2+useI18n 崩溃根治）、ZCode 视觉对齐批（33c0e88 像素规格表+暗色令牌）、zcode 3.14.0 对齐轮（快照守门+归档批量删除+三态裁决）、codex 品牌展示移除、侧栏运行时崩溃根治（自有置顶）、pane-tools 补齐（c24105f）。③**构建与注入修复**——dmg 无签名固化 2777445（本机签名证书已吊销，签出包被 AMFI SIGKILL）、build-dmg clean 步容错 0c5a9f7、inject clean 反向容错 b863d2c、314 头行修正 c345abb、308/309 相撲清偿 280875a。④**评审修复**——09-19 第七轮 3 修复（storage reveal win32 平台分支/git 变更分类）。**门禁终态**：overlay vitest **178 文件 1792 过/0 fail**（v12 新增守门 ~50 例）；clean→verify→inject 全量重放通过（series 259 条含 316/317）；完整 build（openapi→vite→tsc→build-server）EXIT=0；退役名/死代码扫描零残留；**CDP 真机走查六动线全过**（裸 URL 冷启动落 #/app、房间选择→对话画布、循环→运行画布实时|历史、看板跳转、/ide?task= 深链维度绑定、⚙管理台五区实时计数+Esc 三路关闭、最大化/Esc 复原——零产品缺陷；dev 后端单任务写路径降级为环境事实非回归，决策按钮接线由守门测试断言）。**产物**：arm64 dmg **399,552,703B** sha256 `ce35f320a57f21b8…`（无签名 adhoc；`.vite` 零条目 = 281 生效；WorkbenchView chunk 在产且 gov-overlay/flow-nav/ide-dims/s\/room\//ide.dims 全部标记在 dist 资产实测命中）+ win x64 zip **430,594,137B** sha256 `da62c780c1cf052b…`（unzip -t 零错、WorkbenchView chunk×2、`.vite` 零条目；nsis 步 `spawn -86` bad CPU type 失败——历史在案惯例纯 zip 规避，非交付物）。**本机换装**：/Applications 已装本批 dmg（CFBundleShortVersionString 0.7.23 + adhoc/TeamIdentifier=not set + 冷启动进程存活 + :8748 webui 200 + **生产 bundle #/app 三栏工作台完整渲染**[matrix-login 握手后场景条双入口/态势条/三栏/8 会话实测]——2.28 冷启动白屏教训的发版验证项闭环）。

> **2.29 — hermes-studio v0.7.23 升级轮（2026-09-18/19）** — **hermes-studio v0.7.22 → v0.7.23**（2026-09-18 发布，153 文件 +5059/-627：移动端持久终端会话 #3076/#3079、Coding Agent 澄清交互与群聊任务卡 #3080/#3085、会话置顶跨设备同步 #3091、App 通知恢复与 Webhook 订阅 #3090/#3093、ekko-studio npm 包与桌面品牌统一 #3084、DeepSeek 推理重放修复 #3078、MCP HTTP 传输别名 #3081、技能元数据空值 #3083）；hermes-agent v0.21.3 与 element-web v1.12.28 零升级。**patch regen 4**（纯净树试贴 248 hs patch 5 失败 → 双树三方合并再生 → 全新重放 247/247 零失败）：041/042（品牌行上游 Ekko 化随迁，品牌字段保 SwarmStudio + 版本随上游 0.7.23）、087（群聊 autojoin 与上游任务卡改动共存）、267（yml/cjs hunk 随迁；**terminal.ts 部分作废**——上游已把我方 spawn-helper 修复/PowerShell findShell/优雅降级收编进新模块 `services/terminal/runtime.ts`，PTY 会话上限仍由 patch 194 承载）。**新增 patch 2 + 退役 1**：314（server tsconfig exclude custom `__tests__`——在案 45+1 测试类型债不再阻打包链，生产端 custom 仍受 tsc 门禁、测试由 overlay vitest 覆盖）；315（**补上游缺键 `common.close`**——v0.7.23 ImagePreviewOverlay/MessageItem 引用未定义键，我方 i18n-coverage 门禁抓获）——**旋即经 308/309 相撲清偿退役**（并行会话的 308 首 hunk 已含同键，幂等去重后 inject 重放归零，280875a）。**门禁终态**：全新 inject **256/256**（series 257 条，315 退役）；server tsc **0 错**；overlay vitest **178 文件 1830 过/0 fail**；i18n-coverage **18/18**；kanban×3+avatar+matrix×6 焦点组 **91/91**；完整 build（openapi→client→tsc→build-server）EXIT=0；desktop build:main EXIT=0；上游全量 82 败为历史已知 overlay 改造面预期差异（基线同源）。**产物（未上传，用户指令）**：arm64 dmg **399,660,489B** sha256 `ab841c96f9f576d5…`（无签名 adhoc，`.vite` 缺席 = 281 生效；IdeTerminal/MessageQueue/LoopCockpit chunk 在产）+ win x64 zip **430,755,947B** sha256 `2143a723d207c239…`（unzip -t 零错）。**本机换装**：/Applications 已装 0.7.23（CFBundleShortVersionString 0.7.23 + 存活 35s 验证 + :8748 health `webui_version 0.7.23` / gateway running / bridge ready）。**产物基线（诚实注记）**：双产物内容 = main @ cd75e3a（M4c 终端分屏为止）+ 本轮升级（v0.7.23 + regen/314）；其后合入的 ZCode 视觉对齐批（33c0e88，设计令牌/布局形态）与 308/309 相撲清偿（patch 面去重，注入态净内容等价）**不在本批产物内**，随下一构建轮出包。**过程注记**：本轮与并行 IDE 会话共享 checkout，两度触发 electron-builder release/ staging 撞车（ENOENT app.asar）——按「等 pid 退出验产物」纪律化解，最终产物为本会话独立重出。

> **2.27 — matrix-teams 分布式管理轮（2026-09-17）** — 三上游 Latest 复核零升级（hermes-studio **v0.7.22** / hermes-agent **v0.21.3** / element-web **v1.12.28** 均为 GitHub 最新稳定；desktop 捆绑 runtime 0.21.3 不动）。本轮内容 = main 自 2.26（6982fb4）后合流，全部为 matrix-teams 分布式管理面（A 类 custom + i18n patch，零上游骨架 regen）：①**P1 注册房间与 Teams 管理**（af9be8a）——team registry store（发现/创建/写入/监听）+ 事件协议 parser 守门测试 + account tree projection adapter（ownership guard）+ 管理场景 Teams 管理页（d5332ee）+ i18n patch 288/289。②**P2 值守**（f225523/ffc0928）——房间值守分配面板 DutyAssignPanel + store 写入 + 群列表值守徽标 + i18n 290/291。③**P3 任务外派**（51a91a1/dc75f5d）——task dispatch store（聚合外派视图）+ 成员侧接收落地（kv 去重 + 回执）+ 外派列表 UI（轮询回执）+ i18n 292/293。④**终审 Important 修复**（05b7df1）——task-dispatch 监听绑 pinia scope（不随组件卸载）+ 历史回填恢复迟到消息 + 回填旗标按 client 实例维度判定（登出重登后同房间再次回填）。⑤**终审 backlog 九项清偿**（c9ae69e）——死 i18n 键清理/lastError 清错/投影清空/slug 去重/失败 UX/pending 中性文案（i18n 294/295）/in-flight 去重。非打包内容：多用户 Matrix 协作交付模拟 harness + 报告（cb448e7/8fcd4f3，scripts/sim 纯脚本）与报告配图修复（b2965c2）。**门禁**：全新 inject **239/239**（两上游树 pristine 起步，本轮零容错跳过）；server tsc 0 错；overlay vitest **169 文件 1729 过/0 fail**；上游 i18n-coverage 18/18；完整 build（openapi→vue-tsc -b→vite→tsc→build-server）EXIT=0；desktop build:main EXIT=0。**产物（未上传，用户指令）**：arm64 dmg **399,711,512B** sha256 `7553f9ed…c3e3e` + win x64 zip **430,755,857B** sha256 `20ba1022…3116`（均无签名，构建日志 skipped code signing / TeamIdentifier=not set；双产物内 `.vite` 零条目 = patch 281 生效实证；TeamsManagePanel/teams-adapter/IaShell chunk 在产）。**产物对账**：与 2.26 dmg 逐目录 du 对比，除 webui/dist +1.08MB（matrix-teams 内容）外其余目录零差异——dmg 容器 +18.8M 为打包层（HFS 分配/压缩序）波动，非内容增量。**本机换装**：/Applications 已换装本批 dmg（CFBundle 0.7.22 + 存活验证 + webui :8748 `{"status":"ok"}` + 装包 chunk 指纹与构建产物一致）。

> **2.26 — loop 多视图重构 + IDE zcode 对齐轮（2026-09-17）** — 三上游 Latest 复核零升级（hermes-studio **v0.7.22** / hermes-agent **v0.21.3** / element-web **v1.12.28** 均为 GitHub 最新稳定；desktop 捆绑 runtime 0.21.3 不动）。本轮内容 = main 自 2.25（7c8ea29/d8d6972）后三批合流：①**loop graph 多视图重构**（cade7b4，feat/loop-multiview）——LoopCockpitView 瘦身为壳 + 四场景（总览/管理/Code/运维，buildSceneChildren 双挂载单一构造器）：管理场景人员聚合条 + 看板内嵌 + 群栏目弱锚点（未指派 chip 非交互化）、Code 场景任务上下文条 + 文件/终端/上下文 IDE 三区、运维场景告警/分诊 + 值班台 + 快捷动作；终端/文件面板就地参数化（workspacePath 可选 props 缺省回落 store）；任务抽屉拉群弱锚点（建群预填 + 前缀匹配跳群）；i18n patch 283/284（zh/en，282 因撞号顺延空号）。②**09-17 24h 评审第五轮**（76de0b8，7 项高置信缺陷修复 + 4 条守门：ManageScene 筛选隔离、IDE i18n 快照/死选择器/拖拽泄漏、/ide 落点兜底、inject 顶层 realpath、series 207 注释）。③**IDE zcode 3.11.2 对齐缺口**（c1b913f，feat/ide-zcode-parity）：命令面板 + Git 面板 + 1:1 对照表收口（patch 285-287）。**门禁**：全新 inject **231/231**（273 agent 注入态容错跳过）；server tsc 0 错；overlay vitest **1648 过/0 fail**（160 文件中 1 文件为首跑时并行会话 matrix-teams 在途测试写入竞态，复跑 14/14 绿，非 main 内容面）；上游 i18n-coverage 18/18 + kanban/auth/matrix 焦点组 **92/92**；完整 build（openapi→vue-tsc -b→vite→tsc→build-server）EXIT=0；desktop build:main EXIT=0。**产物（未上传，用户指令）**：arm64 dmg **380.9M** sha256 `7c784cf8…2961` + win x64 zip **410.5M** sha256 `d0b7aab7…433a`（无签名 adhoc，TeamIdentifier=not set；装包 webui `.vite` 缓存缺席 = patch 281 生效实证；`loopScenes`/`commandPalette` 内容指纹在产验证）。/Applications 已换装本批 dmg（存活验证 + webui :8748 `{"status":"ok"}` + 装包指纹三验；MCP 进程群全部从新 bundle + 0.21.3 runtime 拉起）。注：构建后 main 另合入 sim 交付模拟 harness（cb448e7——scripts/sim 纯脚本、不进应用包，构建物内容与 main 现 tip 等价）；matrix-teams P1+ 仍在 feat 分支在途，随 2.27+。

> **2.25 — 构建工程修复轮（2026-09-16）** — 三上游 Latest 复核：hermes-studio **v0.7.22** / hermes-agent **v0.21.3** 零升级；element-web 参考实现 **v1.12.27 → v1.12.28**（2026-09-16 发布，无 patch 目标，纯参考基线跟进）。**核心修复：打包体积膨胀根治（patch 281）**——electron-builder 对上游根 `node_modules` 整目录直拷（extraResources filter 无构建缓存排除），980MB 的 `node_modules/.vite` dep-optimizer 缓存被整包扫进 DMG：2.24 轮缺陷产物 dmg 597.6M / x64 zip 622.2M（基线 399M / 430M，+45%）；patch 281 在 `packages/desktop/electron-builder.yml` 的 node_modules 复制过滤链追加 `!**/.vite/**` + `!**/.vite-temp/**`，重建后 dmg 381.1M / zip 410.8M 回归基线，且装包内 `webui/node_modules/.vite` 实测缺席。**同批产物内容修复**：2.24 会话的 0.7.22 构建 bundle 缺整个 ia2 路由树（`ia2.shell` 0 命中、LoopCockpitView/IaShell chunk 缺失 → AI协作中心全链接死）；本批从 main @7c8ea29 全新注入重出，IaShell-I4q_hl2v / LoopCockpitView-aIjQ8nOh / IdeShell-dSVpA_Ch / routes-4Nr_56rv chunk 齐全，`ia2.overview` 路由注册在产；含 main 累积内容——IDE 工作台 /ide（patch 276-280，LoginView-DjI3Wo9R 含 `"/ide"` 落点）、登录页空白修复（patch 249 `{'@'}` 转义 + i18n-message-compile-gate 守门）、serve-server dev 修复（7c8ea29）。**门禁**：全新 inject **226/226**（273 agent 注入态容错跳过）；server tsc 0 错；overlay vitest **146 文件 1592 过/0 fail**；i18n-coverage 18/18 + avatar/kanban 47/47 + matrix ×6 30/30（上轮在案的 matrix-login-session 抖动未复现）；完整 build EXIT=0。**产物（未上传，用户指令）**：arm64 dmg **381.1M** sha256 `fc61590e…0aaef` + win x64 zip **410.8M** sha256 `f300f15b…a1588`（无签名 adhoc，TeamIdentifier=not set；带毒缺陷产物 597.6M/622.2M 归档 release/prev-2.24-defective/）。win nsis 副产物目标在 zip 完成并校验后失败（非交付物，惯例纯 zip 规避 wine）。/Applications 已换装本批 dmg（存活验证 + webui livez + 装包 chunk 指纹三验）。

> **2.24 — hermes-studio v0.7.22 升级轮（2026-09-16）** — hermes-studio **v0.7.21 → v0.7.22**（2026-09-16 02:15 UTC 发布，92 文件 +2432/-201）。核心：**standalone task-plan MCP**（每轮上下文的独立任务计划 MCP，#3053）、**Studio MCP 全局 launch 注入**（coding-agents 全局启动也带 Studio MCP，#3055）、**agent bridge socket 路径过长回退 TCP**（Unix 域套接字路径超限自动回退，长用户名/嵌套目录场景不再起不来）、**Studio-managed Claude OAuth for Hermes Bridge 修复**（#3033）、**DSH plugin manifests 声明版本**、**Ekko MCP HTTP transport 别名归一**、**Ekko Chat tool-result 配对保持**、**OpenRouter 归因统一为共享模块常量**、**grok runtime config 多行值保持**、**Hermes runtime browser bundles 校验**（含 0.21.3 浏览器安装检查模块化适配）、**apikey domain env 正则跨行误匹配修复**、**coding agent PATH 与缺失可执行文件处理**（#3044）、**thinking 指示器显示 agent logos**（#3052）。hermes-agent 仍 **v0.21.3**、element-web 仍 **v1.12.27**（均 Latest 零升级）。**desktop 捆绑 runtime pin 0.21.0 → 0.21.3**（patch 207：tag hermes-0.21.3-runtime / v2026.9.14 / 345cd2b0…；上游 v0.7.22 的 install-hermes 已适配 0.21.3 浏览器检查模块拆分；runtime release 资产 mac-arm64 518MB 等四平台在 EKKOLearnAI/hermes-studio 仓）。**patch 侧**：regen 3 个——042（版本行随上游 0.7.22、品牌字段保持 SwarmStudio）、144（bridge stderr 捕获与上游 OpenRouter 归因共享模块化共存，0 冲突自动合并）、195（fleet-tap 注销块与上游 close() 新增 finishTaskPlanRun abort 清理共存）；207 改名 207-desktop-runtime-pin.patch 并升 pin。纯净 v0.7.22 树全量重放 **0 失败**（214 studio patch 干净套用 + 6 agent patch 路由跳过，agent 树未动）。**门禁**：inject 220/220（manifest 计数；agent 侧 273 因树处注入态已知容错跳过）；server tsc 0 错；overlay vitest **140 文件 1571 过/0 fail**（基线持平）；上游 kanban×3 + auth-routes-avatar + i18n-coverage 62/62（i18n 18/18）；matrix ×6 30/30；上游完整 build（openapi→vue-tsc -b→vite→tsc→build-server）EXIT=0。本轮基线含 main @3ed4bb8（登录落点改 AI 协作中心，patch 274/275）。**追加修复（2026-09-16 晚）**：登录页空白缺陷——patch 249 的 matrix 登录/用户管理文案含裸 `@`（`@user:matrix.example.com`，zh/en × login.matrixUsernamePlaceholder / users.matrixUserIdPlaceholder 共 4 条），vue-i18n 把 `@` 当链接消息语法，登录态过期首次渲染 LoginView 时编译抛 INVALID_LINKED_FORMAT(code 10) 渲染中断 → 整页空白；改为字面量转义 `{'@'}`，并新增全量 locale 消息编译守门测试（i18n-message-compile-gate，11 locale 逐条过 @intlify/message-compiler；注意 baseCompile 不传 onError 会静默丢错，守门必须显式抛）。装机热修 dist 后需清 `~/.hermes-web-ui/cache`（哈希资产 Cache-Control immutable，改内容不改文件名则缓存不失效）。

> **2.23 — hermes-agent v0.21.3 升级轮（2026-09-16）** — hermes-agent **v0.21.2 → v0.21.3**（tag v2026.9.14，2026-09-14 发布：1036 非 merge commits / 2642 文件 / 338 PRs 的 rollup 窗口）。核心：**远程 dashboard 会话刷新风暴不再踢线**（cookie gate 与 desktop bearer 双路对同一轮换 refresh token 合流，唤醒风暴不再触发 reuse 检测整会话吊销）+ **长活进程 state.db 重复写句柄泄漏修复**（gateway/desktop/ACP/CLI 读端只读挂载、写端共享注册表句柄——本机长期挂机场景直受益）；附带 server→client JSON-RPC 与 Pydantic wire-contract 注册表、全模型 reasoning-effort 选择器、OpenRouter OAuth PKCE、HEIF/HEIC/AVIF 解码、MCP OAuth refresh token 绑定 issuer 等（上游明言完整清单随 v0.22.0 归档）。hermes-studio 仍 **v0.7.21**、element-web 仍 **v1.12.27**（GitHub Latest 零升级）；desktop 捆绑 runtime pin 维持 **0.21.0**（patch 207 不动）。**patch 侧**：regen 2 个——118（上游 create_profile 重构为隐藏 staging 目录一次性 rename 发布（防多路复用器收编半成品树），`_seed_run_trace_plugin` 随迁 staging 前置生效；二轮修正：`_ensure_run_trace_enabled_in_config` 裸 `yaml.safe_load` 撞上新 config 读取守卫（raw 读须走 owner 原语），改为 `read_user_config_raw(config_path)` 写回往返，全新树 6 patch 重放 + 守卫 2/2 + profiles 140 复绿）；117（上游 #106342 看板项目作用域 worktree 继承 + workspace 逻辑拆分 kanban_db_workspace.py 并加严 dir 必须绝对路径：overlay 默认 dir 收窄为**普通板生效**（项目板保持上游 scratch→worktree 继承流），dir 无路径时兜底托管 per-task 工作区 `workspaces_root/<id>`）；新增 **273**（4 个上游测试显式钉 `workspace_kind="scratch"`——测试对象是 scratch 清理/回滚语义，不应依赖默认 kind）；inject.mjs 路由扩展 `tests/hermes_cli/` → agent（apply/reverse 双侧）。**门禁**：inject 218/218；server tsc 0 错；overlay vitest **137 文件 1569 过/0 fail**；i18n-coverage 18/18；上游 build（vue-tsc -b + vite）EXIT=0；desktop build:main EXIT=0；agent 焦点组（profiles 140 / kanban CLI+db+boards+board-project+projects 全绿，其中 5 个上游测试在纯净 v0.21.3 树全过、注入树初版全败的回归经 117/273 根治后复绿）；api_server 焦点组 137 过 + 健康探测定向 8 过（patch 250 面）；agent 全量套件实测：跑至 43%+ 零失败后挂死于网关慢段（50 分钟零输出）判停终止；**替代口径 = hermes_cli 全目录 4159 用例：4068 过 / 61 skip / 30 败**，其中 18 个瞬时（静默复跑自愈）、12 个稳定失败经纯净树/补丁重放树双对照定性为**共享工作树环境互作**（config/clamps/env-export/gmi/provider-seam 面，与 patch 目标零交集，双对照树同 venv 全过）；hermes_cli 全目录此前另一轮（重放态）199 过 / 12 败中守卫 1 败即上文 118 真发现，已修。agent .venv 补装 `agent-client-protocol==0.9.0`（v0.21.3 新增 acp 依赖，收集期 11 错→清零）。**并行会话合流**：本轮升级与本机另一 loop-mind-3d 会话共享 checkout 交错推进——其三修复（7babe82/c7e162c/16b7160）落入本 feat 分支、24h 审查五项修复（0c32284：渲染崩溃/资源泄漏/流回收/写入链路/跨端兜底）落 main，merge 176efca 全部并入 2.23 内容（bundle 断言 `lcp-panel__subhead`/`lcp-kpi--alert` 在产验证）。**产物（未上传，用户指令；重建轮）**：首构建不含 0c32284（其提交晚于首构建），已从合并后 main 重建——arm64 dmg 398.7M sha256 `f712bead…648f` + win x64 zip 429.8M sha256 `30f5f229…2807`（无签名 adhoc；首构建 380.0M `ed50c6ac…` / 409.7M `6c72c1a7…` 归档 release/prev-2.23a/）。/Applications 已换装重建轮 dmg（存活验证 + webui :8748 livez ok + LoopCockpitView-DTgNUY3L 新 chunk 实际服务 + TeamIdentifier=not set）。
SwarmStudio **2.21**（基于 hermes-studio v0.7.21 + hermes-agent v0.21.2 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.0）

> **2.22 — 循环驾驶舱重设计为活动思维大脑（2026-09-15）** — 用户裁决：循环驾驶舱是一个**动态增长的活动思维大脑**，不是人工编排的机械控制台。main 演进 a88fc52 → 6b1c12d（merge feat/loop-mind）。**变更**：①中央可视化由 LoopGrowthViz 雷达树图重设计为 **MindViz 思维大脑**——核心神经元（外壳膜波纹 + 核呼吸）居中，思想核（循环/自建图）在皮层散布带稳定哈希定位，每次运行生长为突触末梢神经束（1..3 条有机贝塞尔分支）。②**自适应生长语义**：末梢分支半径随业务阶段权重推进 + 迭代加成壮大；突触可塑性——复用强度（近期运行数）驱动边宽/不透明度渐变，常用通路视觉增强、新突触细如丝；**记忆脉冲**（spark/interrupt/emerge 浮现→褪色）承载时间生长感，事件不是画出来而是涌现后缓慢褪色。③**去人工编排**：驾驶舱移除新建循环向导与编排区主入口（`/app/orchestrate` 路由保留，移出驾驶舱主链）；活大脑只读观察导航——点击思想核/末梢去看运行状态。④退役删除 `growth.ts`/`LoopGrowthViz.vue`/`growth-adapter.test.ts`；新增 `mind.ts`/`MindViz.vue`/`mind-adapter.test.ts`（12 断言：确定性/分组/阶段→半径单调/可塑性/脉冲稳定/边完整性），cockpit-view 更新（含"无人工编排入口"守门）。⑤i18n patch 254/255（loopMind.* zh/en，其余语言 en 回退）。**上游零升级**（v0.7.21 / v0.21.2 / v1.12.27 Latest）。**门禁**：overlay vitest 135 文件 **1519 过/6 skip/0 fail**；i18n-coverage 18/18；上游 build（vue-tsc）EXIT=0；overlay client build EXIT=0 + bundle 断言（lmv chunk 在产、lgv/ia-nav 零残留）。**产物**：mac arm64 dmg + win x64 zip（无签名 adhoc），指纹见 release 注记。
SwarmStudio **2.20**（基于 hermes-studio v0.7.21 + hermes-agent v0.21.2 源码跟踪 + overlay 二次开发；desktop 捆绑 runtime 0.21.0）

> **2.21 — 循环驾驶舱单页重构（2026-09-14）** — 用户裁决：loop graph 页面去多菜单、整合为一个智能驾驶舱单页（动态循环生长图）。main 演进 f260499 → 875e18b（merge feat/loop-cockpit）。**变更**：①`/hermes/loop`（AppSidebar「循环工程图」入口，冻结不动）与 `/app`（登录默认落点）合并渲染同一个 `LoopCockpitView` 单页；退役删除 `LoopSpineView`/`OverviewView`。②IaNav 六区域菜单栏退役；编排/运行/介入/工作项/沟通路由全保留（深链与兼容守卫零变化），改经驾驶舱页头动作区（新建循环/编排/运行中心 + 溢出菜单）与子页 slim 页头（返回驾驶舱）进入。③中央**动态循环生长图**：核心常脉动、循环/自建图落内环、每 run 生长为外向分支且**分支长度 = 业务阶段进度**（阶段推进经 /graph 订阅实时生长）、running 粒子流 + 节点呼吸、完成态绽放、雷达扫描线；`prefers-reduced-motion` 全量降级。④KPI 条（活跃循环/运行中/待介入/7日完成/7日均耗时）+ 介入收件箱 + 循环面板（run/pause/delete 带确认）+ 工作项状态分布 + 今日计划——原总览功能收编不丢失。⑤workspace 流生命周期由 IaShell 移入驾驶舱视图（两挂载点行为一致）。⑥i18n patch 252/253（loopCockpit.* zh/en，其余语言 en 回退）。**上游零升级**（三上游仍为 v0.7.21 / v0.21.2 / v1.12.27 Latest）。**门禁**：overlay vitest 135 文件 **1516 过/6 skip/0 fail**（新增 growth-adapter 11 / cockpit-view 12 / ia-shell 5 组守门，删 ia-nav/overview-components 旧测试）；上游 i18n-coverage 18/18；上游 `npm run build`（含 vue-tsc -b）EXIT=0；overlay client 构建 EXIT=0 且 bundle 断言 LoopCockpitView/IaShell chunk 在产、ia-nav/loop-spine 类零残留；clean/inject 重放 198/198。**产物**：mac arm64 dmg + win x64 zip（双构建物，无签名 adhoc），指纹见 release 注记。
> **2.22 后增量（未发布，2026-09-15 全面排查轮）** — 用户指令：全面排查并解决 Windows 与 Linux（麒麟）平台兼容性问题与 ACP 调用问题。三路并行深扫（ACP 调用链 / 全库平台敏感点 / overlay patch 交集）定位 12 处实际缺陷，落 **patch 265-270（6 个新 patch）**，全部经全新树 clean→inject 215/215 复现。**ACP 调用（coding-agents/dsh，overlay 首次进入该模块）**：265=win32 环境变量过滤大小写归一（`Path`/`windir` 原始大小写被精确匹配 allowlist 静默丢弃）+ ACP stdout 非 JSON 行容错（cmd shim 横幅/杀软注入不再杀整个 turn；64 条上限防静默吞协议破坏）+ pnpm 缺失识别扩 cmd.exe/PowerShell 中英文报错（原 `/pnpm not found|ENOENT/` 在中文 Windows 误报 422）；266=精简麒麟镜像无 `which` 时 findCommandPaths/commandExists 兜底 PATH 直扫（新文件 command-path-scan.ts，含 win32 PATHEXT 展开）+ 桌面 PATH 增补表补 `~/.local/share/pnpm` + Linux 终端候选补 mate-terminal/qterminal/deepin-terminal（麒麟 UKUI 默认 mate-terminal，原报"No supported Linux terminal"）+ scoped runs 目录 hash 64→16 hex（Windows MAX_PATH 减压 48 字符）。**Desktop/打包**：267=node-pty Linux 打包修复——extraResources 增保留 `build/Release/spawn-helper`（Unix pty.spawn 硬依赖，原过滤规则丢弃它 → Linux 内置终端必坏）+ afterPack 增 sharp/@img 平台裁剪与 spawn-helper exec 位恢复 + terminal.ts 修权门槛 darwin→全 POSIX；268=麒麟 AppImage/deb 沙箱回退——无 SUID chrome-sandbox 且禁 unprivileged userns 时 entry.ts 自动 `--no-sandbox` 重启一次（纯决策函数 linux-sandbox.ts，带防循环）；269=Linux 主进程文件日志（原仅 win32；XDG `~/.config/<name>/logs/`）；270=cli-shim Linux GUI 启动无 SHELL 时写 `.bash_profile`/`.bashrc`（原只写 `.profile`，交互 bash 不读 → `ekko-studio` 命令"装了但找不到"）。**测试**：新增 3 守门文件 29 用例（coding-agents-cross-platform 11 / after-pack-combined 12 含临时目录真实执行钩子的集成断言 / linux-sandbox 6）；**修复旧 drift**：node-pty-prebuild-prune.test.ts 自 patch 127 钩子改名后指向不存在文件、纯函数断言长期静默 skip——重写为 after-pack-combined.test.ts。**新目录 custom/upstream-compat/**：跨仓边界测试专用位（custom/server 被符号链接进上游 server/src/custom，其 tsc 视角下相对 import 上游模块解析错位——"custom→upstream import 禁令"的测试侧镜像）。**门禁**：server tsc 0 错（曾拦下测试文件顶层 await 与符号链接 import 两处真问题）+ desktop build:main 0 错 + overlay vitest **139 文件 1569/1569** + i18n-coverage 18/18。**遗留（非代码可解，需构建/发布侧跟进）**：①Linux 运行时资产（hermes-runtime-linux-x64 tarball）从未产出，Linux 包首启下载必 404；②麒麟 V10 glibc 2.28 vs Electron 42/PBS Python 需 ≥2.31，Linux 版须在目标基线构建；③deb 仅 x64，arm64（鲲鹏/飞腾）仅 AppImage（沙箱问题已由 268 缓解）；④electron-updater 在 deb 安装下无法自更新；⑤sharp 为运行时零引用的传递依赖（裁剪属包体卫生非功能修复）。 — 用户裁决：loop graph 页面去多菜单、整合为一个智能驾驶舱单页（动态循环生长图）。main 演进 f260499 → 875e18b（merge feat/loop-cockpit）。**变更**：①`/hermes/loop`（AppSidebar「循环工程图」入口，冻结不动）与 `/app`（登录默认落点）合并渲染同一个 `LoopCockpitView` 单页；退役删除 `LoopSpineView`/`OverviewView`。②IaNav 六区域菜单栏退役；编排/运行/介入/工作项/沟通路由全保留（深链与兼容守卫零变化），改经驾驶舱页头动作区（新建循环/编排/运行中心 + 溢出菜单）与子页 slim 页头（返回驾驶舱）进入。③中央**动态循环生长图**：核心常脉动、循环/自建图落内环、每 run 生长为外向分支且**分支长度 = 业务阶段进度**（阶段推进经 /graph 订阅实时生长）、running 粒子流 + 节点呼吸、完成态绽放、雷达扫描线；`prefers-reduced-motion` 全量降级。④KPI 条（活跃循环/运行中/待介入/7日完成/7日均耗时）+ 介入收件箱 + 循环面板（run/pause/delete 带确认）+ 工作项状态分布 + 今日计划——原总览功能收编不丢失。⑤workspace 流生命周期由 IaShell 移入驾驶舱视图（两挂载点行为一致）。⑥i18n patch 252/253（loopCockpit.* zh/en，其余语言 en 回退）。**上游零升级**（三上游仍为 v0.7.21 / v0.21.2 / v1.12.27 Latest）。**门禁**：overlay vitest 135 文件 **1516 过/6 skip/0 fail**（新增 growth-adapter 11 / cockpit-view 12 / ia-shell 5 组守门，删 ia-nav/overview-components 旧测试）；上游 i18n-coverage 18/18；上游 `npm run build`（含 vue-tsc -b）EXIT=0；overlay client 构建 EXIT=0 且 bundle 断言 LoopCockpitView/IaShell chunk 在产、ia-nav/loop-spine 类零残留；clean/inject 重放 198/198。**产物**：mac arm64 dmg + win x64 zip（双构建物，无签名 adhoc），指纹见 release 注记。
