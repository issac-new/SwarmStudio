 ...
 * 存储：share 记录随会话库 hermes-web-ui.db 的 shared-state 表（kv），零新表。
...
// 进程内 kv（hermes-web-ui.db shared-state 同源表；重启后记录留库——生产读库，MVP 内存兜底）
...
ideSessionShareRouter.post('/api/ide/session-share/create', async (ctx) => {
  ...
  ctx.body = { ok: true, token, url: `/share/${token}`, mode, role: 'host' }
})
