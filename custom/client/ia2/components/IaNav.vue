...
<template>
  <nav class="ia-nav" aria-label="Primary areas">
    ...
    <!-- 用户设置入口：loop graph 首页（/app 壳）直达设置页（/hermes/settings）。
         非 IA 区域（不占 g+数字、不高亮投影），独立类名避免混入区域项断言。 -->
    <button
      type="button"
      class="ia-nav__settings"
      :title="t('ia2.nav.settings')"
      data-ia-nav-settings
      @click="router.push('/hermes/settings')"
    >
      <svg
        class="ia-nav__icon"
        width="16"
      ...
      <span class="ia-nav__label">{{ t('ia2.nav.settings') }}</span>
    </button>
  </nav>
</template>
