...
<!-- 关键指标卡（负责人视角）：近 7 天 run 成功率 / 平均耗时 / 熔断次数。
     ...
     口径标注（Task 4 审查 B-2）：平均耗时是采样均值（副注"基于 N 个样本"）；
     熔断计数在 loop 事件采集降级时标"部分数据"（stuckPartial）。 -->
...
/** 耗时样本副注：有均值才标注（无样本落 — 无口径可注） */
const durationNote = computed(() => {
  if (!props.metrics || props.metrics.avgDurationMs === null) return ''
  return t('ia2.overview.metricDurationSamples', { n: props.metrics.durationSamples })
})
...
/** 熔断计数降级副注：loop 事件采集不完整（MetricsRaw.partial）时标部分数据 */
const stuckNote = computed(() => {
  if (!props.metrics || !props.metrics.stuckPartial) return ''
  return t('ia2.overview.metricPartial')
})
</script>
...
<template>
  <div class="ia-card ia-card--static ia-metrics" data-testid="ia-metrics">
    ...
    <div class="ia-metrics__rows">
      ...
      <div v-if="durationNote" class="ia-metrics__note">{{ durationNote }}</div>
      <div class="ia-metrics__row">
        <span class="ia-metrics__key">{{ t('ia2.overview.metricStuck') }}</span>
        <span class="ia-metrics__val">{{ stuckLabel }}</span>
      </div>
      <div v-if="stuckNote" class="ia-metrics__note">{{ stuckNote }}</div>
    </div>
  </div>
</template>
