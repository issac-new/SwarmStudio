<!-- overlay/custom/client/ia2/views/scenes/CollabScene.vue -->
<!-- 协作场景（2026-09-18 统一导航重构 Task 3）：旧 AI协作中心（CockpitView 三栏）
     迁入 /app/collab，作为 IaShell 下的场景页。
     拆除：CockpitTopBar（页头上移 IaShellHeader）、LoopModal（loop 家族退役）、
     Notify/Schedule/RunTrace 弹窗（壳级 IaShell 已挂）、bootstrap/disconnect
     （壳级幂等武装）；cockpit.scss 改由 ia2/index.ts 顶部同步导入。
     保留：三栏（看板 / 协作图+时序流 / 工作区模式切换）+ History/TemplateManager/
     titleDetail/kanbanDetail/swarmKanban 弹窗群；右栏经子路由嵌入
     chat/history/global-agent 页（ia2.collab*，词表见 collab-embed-routes.ts）。
     新增：场景顶部 slim 工具条（日程 + Run Observatory，壳级弹窗的场内入口）。 -->
...
import { isCollabEmbedRoute } from './collab-embed-routes'
import { useI18n } from 'vue-i18n'

const store = useCockpitStore()
// 日程弹窗（CockpitScheduleModal）壳级已挂，开关走 ia2 workspace store
// （数据源/选中日期/✕/Esc 关闭同源——2026-09-12 审查 P1 的约束不变）。
const workspace = useWorkspaceStore()
const { t } = useI18n()
const router = useRouter()
const route = useRoute()

// 当前是否在嵌入子路由（决定右栏显示 router-view 还是 cockpit 工作区）；
// 词表单一事实源：collab-embed-routes.ts（CockpitModeBar 共用）
const isChatSubRoute = computed(() => isCollabEmbedRoute(route.name as string))

// 看板「进入中心」→ 工作项场景（旧 swarm-kanban 顶层路由已退役）
const goCenter = () => router.push({ name: 'ia2.tasks' })
...
<template>
  <div class="cockpit" data-testid="scene-collab">
    <!-- 场景 slim 工具条：壳级弹窗（日程 / Run Observatory）的场内入口 -->
    <div class="collab-tools" data-testid="collab-tools">
      <button type="button" class="collab-tools__btn" data-testid="collab-schedule" @click="workspace.openSchedule()">
        {{ t('cockpit.schedule') }}
      </button>
      <button type="button" class="collab-tools__btn" data-testid="collab-runtrace" @click="store.openRunTraceGlobal()">
        {{ t('cockpit.runObservatory') }}
      </button>
    </div>
    <CockpitAttention />
    ...
    <div class="cockpit__body" :class="{ 'has-max': store.maximized.left || store.maximized.mid || store.maximized.right }">
      ...
      <section class="cockpit-col cockpit-col--right"
        ...
        <div class="cockpit-col__inner">
          ...
          <!-- 嵌入子路由：app 页面（chat/history/global-agent + group/workflow 保留集） -->
          ...
          <CockpitFleetGrid v-else-if="store.workspaceMode === 'fleet'" />
...
.collab-tools__btn {
  ...
  border-radius: var(--radius-standard, 6px); padding: 3px 14px; cursor: pointer;
...
/* 嵌入的 app 页面（chat/history/global-agent 等）填满右栏 */
.cockpit-embed-view {
  flex: 1;
  min-height: 0;
  overflow: hidden;
