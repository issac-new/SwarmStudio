...
const chartOption = computed(() => {
  ...
  const data = props.nodes.filter(n => vis.has(n.id)).map(n => {
    ...
    return {
      ...
      // 矩形节点，文字显示在内部
      symbol: 'roundRect',
      symbolSize: isSelected ? [190, 64] : isHit ? [180, 60] : [170, 56],
      itemStyle: {
        color: '#ffffff',
        borderColor: isSelected ? '#ff6600' : (KIND_COLOR[n.kind] ?? '#999'),
      ...
      label: {
        ...
        rich: {
          ...
          status: { fontSize: 9, color: '#888', padding: [0, 0, 0, 4] },
          title: { fontSize: 11, color: '#222', fontWeight: 'bold', lineHeight: 16 },
          task: { fontSize: 9, color: '#666', lineHeight: 14, fontFamily: 'monospace' },
        },
      },
      _nodeId: n.id,
    }
